...
def run_utonia_on_fused(backbone, points, dino_feats, num_pts=16384):
    print(f"🧠 Running Utonia Geometry Extraction on {num_pts} points...")
    pts = points.copy()
    pts -= pts.mean(0); pts /= (np.max(np.linalg.norm(pts, axis=1)) + 1e-8)
    
    idx = np.random.choice(len(pts), num_pts, replace=(len(pts) < num_pts))
    pts_s = pts[idx]
    
    u_input = torch.from_numpy(pts_s).float().unsqueeze(0).cuda()
    u_feats = get_utonia_features(backbone, u_input)
    
    # Matching DINO features to the same indices
    d_feats = dino_feats[idx]
    return pts_s, u_feats, d_feats
...
if __name__ == "__main__":
    ...
    raw_pts, raw_dino = build_global_object([0, 10, 20, 30, 40], PATHS, sam, dino, [[470, 370]])
    f_pts, f_dino = voxel_fusion(raw_pts, raw_dino)

    s_pts, u_feats, d_feats = run_utonia_on_fused(uto, f_pts, f_dino)
    ...
    save_barcode(global_embedding, "desk_final_identity.png")
    
    print(f"\n🚀 SUCCESS: Identity barcode shape {global_embedding.shape} is ready for Phase 2: Decoding.")